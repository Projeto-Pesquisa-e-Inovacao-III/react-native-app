...
function IconPlans({ active }: { active: boolean }) {
  ...
  return (
    <Svg width={25} height={24} viewBox="0 0 25 24" fill="none">
      ...
      />
      <Path
        d="M20.5 3H15.5C14.9477 3 14.5 3.44772 14.5 4V9C14.5 9.55228 14.9477 10 15.5 10H20.5C21.0523 10 21.5 9.55228 21.5 9V4C21.5 3.44772 21.0523 3 20.5 3Z"
        stroke={c}
        strokeWidth={2}
        strokeLinecap="round"
      />
      <Path
        d="M20.5 14H15.5C14.9477 14 14.5 14.4477 14.5 15V20C14.5 20.5523 14.9477 21 15.5 21H20.5C21.0523 21 21.5 20.5523 21.5 20V15C21.5 14.4477 20.5 14 20.5 14Z"
        stroke={c}
        strokeWidth={2}
        strokeLinecap="round"
      />
...
export default function BottomTabBar({ activeTab, onTabPress, userRoles }: Props) {
  const insets = useSafeAreaInsets();
  const bottomInset = Math.max(insets.bottom, 0);
  const press = (tab: TabName) => onTabPress?.(tab);

  const isPersonalOrAdmin = userRoles?.includes('personal') || userRoles?.includes('admin');
  ...
  return (
    <View style={[styles.bar, { height: 60 + bottomInset, paddingBottom: bottomInset }]}>
      ...
      {isPersonalOrAdmin ? (
        ...
        <TabItem
          icon={<IconPlans active={activeTab === 'plans'} />}
          label="Planos"
